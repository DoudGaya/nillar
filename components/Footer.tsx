...
export const Footer = () => {
  return (
    <div className=' w-full bg-banner border-t-2 dark:bg-stone-800 border-line-color dark:border-stone-500'>
      <div className=" max-w-5xl mx-auto flex flex-col">
        ...
        <div className=" grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 border-t py-8 border-line-color dark:border-stone-500">
        {
            navItems.map((s) => {
              return <div className="">
                 <div className=" px-4">
                  <h2 className=' uppercase py-2 font-header mb-6 border-b border-line-color dark:border-stone-500 text-2xl'>{s.header}</h2>
                  <div className=" flex flex-col space-y-1">
                    {
                    s.navLinks.map((link) => {
                      return <Link key={link.id} className=' font-keisei hover:underline' href={link.url}> {link.name} </Link>
                    } )
                  }
                  </div>
                  </div>
