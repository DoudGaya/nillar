    ...
    <div className="max-w-4xl mx-auto px-10">
      ...
      <div className=' flex h-full flex-col lg:flex-row'>
          ...
          <div className=" lg:w-3/5 py-10 h-full flex flex-col space-y-6 bg-[#F7F7F7] dark:bg-stone-800 px-6 ">
                    <Link href={`/${featured.slug.current}`} className=' font-header hover:underline font-semibold text-2xl'>{ featured.title }</Link>
              <div className=" text-justify font-newsreader line-clamp-4 ">
                {featured.overview}
              </div>
              <div className="">
                  <Link href={`/${featured.slug.current}`} className=' hover:border bg-primary-light dark:bg-stone-600 px-6 py-2 font-keisei'> Read More. </Link>
              </div>
